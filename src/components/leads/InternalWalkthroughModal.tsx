...
export const InternalWalkthroughModal: React.FC<InternalWalkthroughModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm overflow-y-auto">
      <div className="relative w-full max-w-xl bg-slate-900 border border-slate-700/80 rounded-2xl shadow-2xl shadow-black/80 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center justify-between px-6 py-4 bg-slate-800/80 border-b border-slate-700">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-white tracking-tight">Schedule On-Site Walkthrough</h2>
              <p className="text-xs text-slate-400">
                Facility inspection for <span className="text-slate-200 font-medium">{lead.companyName}</span> ({lead.leadId})
              </p>
            </div>
          </div>
          <button
        ...
        <form onSubmit={handleSave} className="p-6 space-y-4">
          {errorMsg && (
            <div className="flex items-center gap-2 p-3 text-xs text-rose-300 bg-rose-950/50 border border-rose-800 rounded-xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-medium text-slate-300 mb-1">Walkthrough Status</label>
              <select
            ...
            <div>
              <label className="block text-xs font-medium text-slate-300 mb-1">Assigned Sales Rep</label>
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-3 py-2 text-sm bg-slate-800 border border-slate-700 rounded-xl text-white focus:outline-none focus:border-emerald-500"
                />
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-medium text-slate-300 mb-1">Walkthrough Date</label>
              <div className="relative">
          ...
          <div>
            <label className="block text-xs font-medium text-slate-300 mb-1">Meeting & Access Instructions</label>
            <input
              type="text"
              ...
              className="w-full px-3 py-2 text-sm bg-slate-800 border border-slate-700 rounded-xl text-white placeholder-slate-500 focus:outline-none focus:border-emerald-500"
            />
          </div>

          <div>
            <label className="block text-xs font-medium text-slate-300 mb-1">Walkthrough Notes & Frustrations</label>
            <textarea
              rows={3}
              ...
              className="w-full px-3 py-2 text-sm bg-slate-800 border border-slate-700 rounded-xl text-white placeholder-slate-500 focus:outline-none focus:border-emerald-500 resize-none"
            />
          </div>
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-800">
            ...
            <button
              type="submit"
              disabled={isSaving}
              className="px-5 py-2 text-sm font-semibold text-white bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 rounded-xl shadow-lg shadow-emerald-600/30 transition-all flex items-center gap-2"
            >
              {isSaving ? (
                <>
